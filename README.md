...
- **Biometric / access-control enrollment over BLE**: card / fingerprint / passcode / face / palm enroll on Touch / Touch Pro / Face / Palm (`SesameBle#biometric`); `registerDelegate` also surfaces the device's non-enroll publishes — Touch Pro `mechStatus`, battery voltage, child-key slots (`PUB_KEY_SESAME`), the unsupported-slot flag, and BLE TX power. The **read-only subset** (list enrolled cards / passcodes / fingerprints / faces / palms, get the current enroll mode) is also wired to the CLI as `sesame ble cards/passcodes/fingers/faces/palms/mode`; enrollment (add/delete/rename, mode-set) stays library-only
- **SESAME Bike3 fingerprint over BLE**: list / delete / rename fingerprints and get/set enroll mode on Bike3 (`SesameBle#fingerPrint`) — Bike3 is Bike2 (unlock) plus a fingerprint capability, so only the fingerprint subset is exposed (the list / mode-get reads are reachable from the CLI via `sesame ble fingers/mode`; delete / rename / mode-set stay library-only)
- **SESAME Bot2 / Bot3 scripts over BLE**: run a script by index, select the active script, read the current script, list script names, and write a script (`SesameBle#script`) — the read-only subset (list script names + current script) is also available via `sesame ble script`; select / write / run-by-index stay library-only
