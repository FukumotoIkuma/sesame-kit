...
// fixtures は kit パッケージ配下 (2 つ上)。
const REPO_ROOT = join(__dirname, "..", "..", "..", "..");
const KIT = join(__dirname, "..", "..");
const SCRIPT = join(REPO_ROOT, "scripts", "canary-upstream.mjs");
const FIXTURES_DIR = join(KIT, "tests", "fixtures", "upstream");
...
describe("upstream canary — offline replay", () => {
  ...
  it("スキーマ違反 fixture を入れると drift を検出して exit 1", () => {
    ...
    try {
      // スクリプトは自身 (tmp/scripts/canary-upstream.mjs) から相対で
      //   ../packages/kit/tests/fixtures/upstream  (fixtures) と
      //   ../packages/kit/src/serve/result-schemas.js  (スキーマ) を参照する。
      // → tmp 配下に同じ packages/kit ツリーを再現する。
      const tmpFixtures = join(tmp, "packages", "kit", "tests", "fixtures", "upstream");
      ...
      cpSync(join(KIT, "src"), join(tmp, "packages", "kit", "src"), { recursive: true });
