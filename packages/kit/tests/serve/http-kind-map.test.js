...
const ROOT = resolve(__dirname, "..", ".."); // kit パッケージルート (fixtures/clients はここ配下)
const REPO_ROOT = resolve(ROOT, "..", ".."); // リポジトリルート (scripts/ はここ配下)
...
describe("SURF-10: HTTP→kind 写像 (生成テンプレート)", () => {
  ...
  it("gen-sdk-ts.mjs のテンプレートに fixture の全エントリが現れる", () => {
    const src = readFileSync(resolve(REPO_ROOT, "scripts", "gen-sdk-ts.mjs"), "utf8");
  ...
  it("gen-sdk-py.mjs のテンプレートに fixture の全エントリが現れる", () => {
    const src = readFileSync(resolve(REPO_ROOT, "scripts", "gen-sdk-py.mjs"), "utf8");
  ...
  it("4 実装すべてに出典コメント (http-kind-map.json) がある", () => {
    ...
    for (const [base, ...rel] of [
      ...
      [ROOT, "clients", "js", "sesame-client.mjs"],
...
describe("SURF-21: clients/js の SesameErrorKind が serve の 7 kind と一致", () => {
  it("d.ts の union に serve KIND の全値が載っている", async () => {
    ...
    const { KIND } = await import("@sesame-kit/core/jsonrpc");
